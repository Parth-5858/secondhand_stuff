...
function deletebrand(bid){
	$.ajax({
			url: "include/allfunction.php?action=delete_brand",
			type: "post",
			data: {bid:bid},
			success: function (response) {
				//console.log(response);
				...
				if(response==1){							
					toastr.success('Brand Successfully Deleted!', 'Mital Jewels!');
					location.reload();							
				}	
				
				else{
					toastr.error("Opps! Couldn't Deleted Brand, Please Refresh page!", 'Mital Jewels!');
